...
Page({
      ...
      success(res) {
        if (res.confirm) {
          ...
          wx.cloud.callFunction({
            name: 'ranking-list-2020-sk-check',
            success: res => {
  ...
  buttonSearchStartWhitoutSignUp: function (e) {
    ...
    // 提交数据
    wx.cloud.callFunction({
      name: 'ranking-list-2020-sk-sign-up',
      data: {
  ...
  bindShowDetailTap: async function (e) {
    ...
    // 获取当前用户已经晒分的岗位
    let myScore = await db.collection("RankingListScore2020SK").where({ Phone: this.data.Phone }).get()
  ...
  getDetail: async function (department, position) {
    ...
    // 获取当前用户已经晒分的岗位
    let listData = await db.collection("RankingListScore2020SK").where({ Department: department, Position: position }).orderBy('Score', 'desc').get()
  ...
  postRank: function () {
    ...
    // 提交报名信息
    const db = wx.cloud.database()
    db.collection("RankingListScore2020SK").where({ Phone: this.data.Phone }).get()
      .then(res => {
        if (res.errMsg !== "collection.get:ok") {
          ...
          if (res.data.length > 0) {
            // 提交过, 进行更新操作
            db.collection("RankingListScore2020SK").where({ Phone: this.data.Phone }).update({
              data: {
            ...
            // 没有提交过, 进行插入操作
            db.collection('RankingListScore2020SK').add({
              data: {
                Phone: this.data.Phone,
                Department: this.data.DetailData.Position.danwei_code,
                Position: this.data.DetailData.Position.zhiwei_code,
