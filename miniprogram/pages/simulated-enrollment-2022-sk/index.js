// pages/simulated-enrollment-2022-sk/index.js
const app = getApp()
...
Page({
      ...
      success(res) {
        if (res.confirm) {
          ...
          wx.cloud.callFunction({
            name: 'simulated-enrollment-2022-sk-check',
            success: res => {
              if (res.errMsg === "cloud.callFunction:ok" && res.result.code === 0) {
                ...
                // 获取检索条件
                wx.request({
                  url: 'https://app.offcn.com/zwsk/wechat/zhiwei/index?sign=b4a8b2f7z4d3n6o0hs',
                  data: {
                    zwcode: "jl",
  ...
  buttonSearchStartWhitoutSignUp: function (e) {
    ...
    // 提交数据
    wx.cloud.callFunction({
      name: 'simulated-enrollment-2022-sk-sign-up',
      data: {
  ...
  buttonSearchStart: function () {
    // 检索职位
    wx.request({
      url: 'https://app.offcn.com/zwsk/wechat/zhiwei/search?sign=b4a8b2f7z4d3n6o0hs',
      data: {
        zwcode: "jl",
  ...
  bindShowDetailTap: async function (e) {
    ...
    // 获取同岗位报名人数
    let enrollmentCount = await db.collection('SimulatedEnrollment2022SKPosition').where({
    ...
    // 获取当前用户报名的岗位
    let enrollmentMyPosition = await db.collection("SimulatedEnrollment2022SKPosition").where({
    ...
    // 获取官方数据
    let offical = await db.collection("SimulatedEnrollment2022SKOfficial").where({
      Department: this.data.PositionData.lists[e.currentTarget.dataset.index].danwei_code,
      Position: this.data.PositionData.lists[e.currentTarget.dataset.index].zhiwei_code,
    }).get()
    ...
    if (this.data.DetailData.Enrollment.myPosition !== null) {
      ...
      wx.showModal({
        ...
        success(res) {
          if (res.confirm) {
            const db = wx.cloud.database()
            db.collection("SimulatedEnrollment2022SKPosition").where({ Phone: that.data.Phone }).remove()
              .then(res => {
                console.log(res)
  ...
  positionSignUp: function () {
    ...
    // 提交报名信息
    const db = wx.cloud.database()
    db.collection('SimulatedEnrollment2022SKPosition').add({
      data: {
        Phone: this.data.Phone,
      ...
      .then(res => {
        if (res.errMsg !== "collection.add:ok") {
          ...
          // 重新获取报名状态
          db.collection('SimulatedEnrollment2022SKPosition').where({
            Department: this.data.DetailData.Position.danwei_code,
            Position: this.data.DetailData.Position.zhiwei_code,
          })
            .count().then(res => {
  ...
  onShareAppMessage: function () {
    return {
      title: '2022省考同岗位报名人数及竞争比查询',
      imageUrl: 'http://jl.offcn.com/zg/ty/images/exam-helper/simulated-enrollment/2022-sk-share.jpg'
    }
  },
