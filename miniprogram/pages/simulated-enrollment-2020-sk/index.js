// pages/simulated-enrollment-2020-sk/index.js
const app = getApp()
...
Page({
      ...
      success(res) {
        if (res.confirm) {
          ...
          wx.cloud.callFunction({
            name: 'simulated-enrollment-2020-sk-check',
            success: res => {
  ...
  bindShowDetailTap: async function (e) {
    ...
    // 获取同岗位报名人数
    let enrollmentCount = await db.collection('SimulatedEnrollment2020SKPosition').where({
    ...
    // 获取当前用户报名的岗位
    let enrollmentMyPosition = await db.collection("SimulatedEnrollment2020SKPosition").where({
    ...
    // 获取官方数据
    let offical = await db.collection("SimulatedEnrollment2020SKOfficial").where({
      Department: this.data.PositionData.lists[e.currentTarget.dataset.index].danwei_code,
      Position: this.data.PositionData.lists[e.currentTarget.dataset.index].zhiwei_code,
    }).get()
    ...
    if (this.data.DetailData.Enrollment.myPosition !== null) {
      ...
      wx.showModal({
        ...
        success(res) {
          if (res.confirm) {
            const db = wx.cloud.database()
            db.collection("SimulatedEnrollment2020SKPosition").where({ Phone: that.data.Phone }).remove()
              .then(res => {
                console.log(res)
  ...
  positionSignUp: function () {
    ...
    // 提交报名信息
    const db = wx.cloud.database()
    db.collection('SimulatedEnrollment2020SKPosition').add({
      data: {
        Phone: this.data.Phone,
      ...
      .then(res => {
        if (res.errMsg !== "collection.add:ok") {
          ...
          // 重新获取报名状态
          db.collection('SimulatedEnrollment2020SKPosition').where({
            Department: this.data.DetailData.Position.danwei_code,
            Position: this.data.DetailData.Position.zhiwei_code,
          })
            .count().then(res => {
  ...
  onShareAppMessage: function () {
    return {
      title: '2020省考同岗位报名人数及竞争比查询',
      imageUrl: 'http://jl.offcn.com/zg/ty/images/exam-helper/simulated-enrollment/2020-sk-share.jpg'
    }
  }
})