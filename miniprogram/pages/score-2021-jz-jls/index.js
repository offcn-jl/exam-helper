...
Page({
  data: {
    ...
    banner_bk:"http://jl.offcn.com/zg/ty/images/exam-helper-mini-program/position/2021/score-2021-jz-jls-index.jpg",// 背景图片
    ...
    CRMRemark: "活动编码:HD202107270156,活动表单ID:94858", // CRM 注释  小程序-2021年吉林市教师招聘晒分数知分差
        ...
        .get({
          success:res=>{
            ...
            if(res.data.length > 0){
              ...
              db.collection("score-2021-jz-jls").where({_openid:_this.data.openid}).count({
                success: res =>{
                  console.log(res)
                  _this.setData({
                    result:res.total
                  ...
                  if(_this.data.result == "1"){ //如果result是空，表示openid没有在score-2021-jz-jls里
                    db.collection("score-2021-jz-jls").add({ //将新用户的手机号录入score-2021-jz-jls
                      data:{
                    ...
                    // 对原来数据进行修改
                    // db.collection("score-2021-jz-jls").where({_openid:_this.data.openid}) .update({
                    //   data: {
                    //     city:_this.data.cityValue,
                    //     county:_this.data.countyValue,
                    //     subject:_this.data.subjectValue,
