...
exports.main = async (event, context) => {
  ...
  return db.collection("SimulatedEnrollment2021SK").where({
    ...
    if (res.errMsg == 'collection.get:ok') {
      if (res.data.length != 0) {
        ...
        return rp({
          ...
          json: true,
          body: {
            "CRMSID": "4750c9d77f15d37ea197a6467df45f5a",
            "Suffix": event.Suffix,
            "Phone": event.cloudID.data.phoneNumber,
            "Remark": "2021吉林省考模拟报名,表单ID:73725,活动编码:HD202102170091"
          },
          method: 'POST'
        }).then((res) => {
          if (res.Code === 0) {
            // 推送成功, 进行后续操作
            // 插入数据
            return db.collection("SimulatedEnrollment2021SK").add({
              data: {
                OpenID: wxContext.OPENID,
