...
exports.main = async (event, context) => {
  ...
  return db.collection("SimulatedEnrollment2022SK").where({
    ...
    if (res.errMsg == 'collection.get:ok') {
      if (res.data.length != 0) {
        ...
        return rp({
          ...
          json: true,
          body: {
            "CRMSID": "56a0de1b86f6a7f301d0c62f7f1597f1",
            "Suffix": event.Suffix,
            "Phone": event.cloudID.data.phoneNumber,
            "Remark": "2022吉林省考模拟报名,表单ID:103585,活动编码:HD202110251261"
          },
          method: 'POST'
        }).then((res) => {
          if (res.Code === 0) {
            // 推送成功, 进行后续操作
            // 插入数据
            return db.collection("SimulatedEnrollment2022SK").add({
              data: {
                OpenID: wxContext.OPENID,
